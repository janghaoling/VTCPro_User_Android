...
import com.vtcpro.user.common.ConnectivityReceiver;
import com.vtcpro.user.common.Constants;
import com.vtcpro.user.common.CustomDialog;
import com.vtcpro.user.common.LocaleHelper;
import com.vtcpro.user.data.SharedHelper;
import com.vtcpro.user.ui.activity.OnBoardActivity;
import com.vtcpro.user.ui.activity.login.EmailActivity;
import com.vtcpro.user.ui.activity.login.PasswordActivity;
import com.vtcpro.user.ui.activity.register.RegisterActivity;
import com.vtcpro.user.ui.activity.social.SocialLoginActivity;
import com.vtcpro.user.ui.activity.splash.SplashActivity;
import com.vtcpro.user.ui.countrypicker.Country;
    ...
    protected JSONObject getAddressJson(LatLng latLng) {
        ...
        try {
            ...
            if ((addresses != null) && !addresses.isEmpty()) {
                ...
                Log.d("LOCATION=>", "Address " + " lat " + latLng.latitude + " lng " + latLng.longitude +
                        "\n--- " + add +
                        "\n country " + obj.getCountryName() +
                        "\n country code " + obj.getCountryCode() +
                        "\n area " + obj.getAdminArea() +
                        "\n postal code " + obj.getPostalCode() +
                        "\n sub admin area " + obj.getSubAdminArea() +
                        "\n locality " + obj.getLocality() +
                        "\n sub through fare " + obj.getSubThoroughfare() +
                        "\n feature " + obj.getFeatureName() +
                        "\n extra " + obj.getExtras());
...
//
//        final Intent intent = new Intent(BaseActivity.this, AccountKitActivity.class);
...
//                            Toasty.error(this, getString(R.string.internal_server_error), Toast.LENGTH_SHORT).show();
    ...
    private String getErrorMessage(JSONObject jsonObject, String error) {
        try {
            ...
            else return getStringResourceByName(error);
        } catch (Exception e) {
            e.printStackTrace();
            return getString(R.string.something_went_wrong);
        }
        return getStringResourceByName(error);
    }
    ...
    private String getStringResourceByName(String aString) {
        ...
        if (aString.equalsIgnoreCase("No Drivers Found"))
            resId = R.string.no_drivers_found;
        else if (aString.equalsIgnoreCase("Updated Successfully"))
            resId = R.string.updated_successfully;
        else if (aString.equalsIgnoreCase("Ride Scheduled"))
            resId = R.string.ride_scheduled;
        else if (aString.equalsIgnoreCase("Ride Cancelled"))
            resId = R.string.ride_cancelled;
        else if (aString.equalsIgnoreCase("Already Ride Cancelled"))
            resId = R.string.already_ride_cancelled;
