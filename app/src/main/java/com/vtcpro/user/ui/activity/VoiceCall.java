...
public class VoiceCall extends AppCompatActivity {
    ...
    private final IRtcEngineEventHandler mRtcEventHandler = new IRtcEngineEventHandler() { // Tutorial Step 1
        ...
        public void onUserJoined(int uid, int elapsed) {
            ...
            if(mConnecting.getVisibility() == View.VISIBLE){
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mPlayer = MediaPlayer.create(this, R.raw.skype_incoming);
        mPlayer.setLooping(true);

        mConnecting = findViewById(R.id.ringing_text);
        mConnecting.setVisibility(View.VISIBLE);

        mAvatar = findViewById(R.id.avatar);
        ((TextView)findViewById(R.id.username_text)).setText(DATUM.getProvider().getFirstName());
        ...
        Glide.with(this).
                ...
                apply(RequestOptions.placeholderOf(R.drawable.ic_user_placeholder).
                        dontAnimate().error(R.drawable.ic_user_placeholder)).into(mAvatar);

    }

    private void initAgoraEngineAndJoinChannel() {
        Boolean connected = getIntent().getBooleanExtra("connected", false);
        if(mPlayer != null && connected == false){
            mPlayer.start();
            mConnecting.setVisibility(View.VISIBLE);
        }else{
            mConnecting.setVisibility(View.INVISIBLE);
    ...
    private void joinChannel() {
        String accessToken = null;//getString(R.string.agora_access_token);
        ...
        mRtcEngine.joinChannel(accessToken, "channeluser" + DATUM.getUserId()  + "provider"+ DATUM.getProviderId(), "Extra Optional Data", 0); // if you do not specify the uid, we will generate the uid for you
    }
