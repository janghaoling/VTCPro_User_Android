...
import com.vtcpro.user.common.EqualSpacingItemDecoration;
...
import com.vtcpro.user.data.network.model.EstimateFare;
...
import com.vtcpro.user.ui.adapter.ServiceAdapter;
...
import static com.vtcpro.user.common.Constants.RIDE_REQUEST.CALCULATE_STATE;
...
import static com.vtcpro.user.common.Constants.RIDE_REQUEST.DISTANCE_VAL;
import static com.vtcpro.user.common.Constants.RIDE_REQUEST.PAYMENT_MODE;
import static com.vtcpro.user.common.Constants.RIDE_REQUEST.SERVICE_TYPE;
...
import static com.vtcpro.user.common.Constants.RIDE_REQUEST.TOTAL_PRICE;
import static com.vtcpro.user.data.SharedHelper.getKey;
import static com.vtcpro.user.data.SharedHelper.getProviders;
import static com.vtcpro.user.data.SharedHelper.putKey;
...
//                if (ServiceTypesFragment.this.isVisible()) {
                ...
                if (response.body() != null) {
                    ...
                    Double fare = 0.0;
                    if (Double.compare(estimateFare.getEstimatedFare(), estimateFare.getService().getMin_price()) < 0 ||
                            Double.compare(estimateFare.getEstimatedFare(), estimateFare.getService().getMin_price()) == 0) {
                        fare = Double.sum(selectedService.getFixed(), estimateFare.getTaxPrice());
                        estimateFare.setEstimatedFare(fare);
                    } else {
                        fare = selectedService.getFixed();
                        estimateFare.setEstimatedFare(fare);
                    }
...
//                        RIDE_REQUEST.put(TOTAL_PRICE, selectedService.getFixed());
                        RIDE_REQUEST.put(TOTAL_PRICE, fare);
