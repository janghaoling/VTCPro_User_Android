...
import com.vtcpro.user.data.network.model.Payment;
import com.vtcpro.user.data.network.model.User;
import com.vtcpro.user.ui.activity.main.MainActivity;
...
import static com.vtcpro.user.common.Constants.RIDE_REQUEST.CARD_ID;
import static com.vtcpro.user.common.Constants.RIDE_REQUEST.CARD_LAST_FOUR;
...
import static com.vtcpro.user.data.SharedHelper.getKey;
import static com.vtcpro.user.ui.activity.payment.PaymentActivity.PICK_PAYMENT_METHOD;
...
public class InvoiceFragment extends BaseFragment implements InvoiceIView {
    ...
    @BindView(R.id.peak_hours)
    TextView tvPeakHours;
...
//            } else llTravelTime.setVisibility(View.GONE);
        ...
        initPaymentView(datum.getPaymentMode(), "", false, datum.getUseWallet());
...
//                            getNewNumberFormat(payment.getFixed())));
                ...
                if (payment.getDiscount() > 0) {
                    Double totalValue = payment.getFixed() + payment.getTax() + payment.getPeakAmount() + payment.getDistance();
                    total.setText(getNewNumberFormat(totalValue));
                } else {
                    total.setText(getNewNumberFormat(payment.getTotal()));
                }
                tvPeakHours.setText(getNewNumberFormat(payment.getPeakAmount()));
...
//                if (payment.getDiscount() > 0) {
                    ...
                    tvDiscount.setText(getNewNumberFormat(payment.getDiscount()));
...
//                                getNewNumberFormat(payment.getHour())));
                ...
                Toast.makeText(getActivity().getApplicationContext(), getString(R.string.payment_is_null),
    ...
    public void onResume() {
        ...
        if (datum.getPaid() == 0 && payingMode.equalsIgnoreCase("CASH")) {
            ...
            if (user.getUserType().equals(USER_TYPE_COMPANY) && datum.getUseWallet() == 1) {
                return;
            }
        ...
        } else if (datum.getPaid() == 0 && !payingMode.equalsIgnoreCase(Constants.PaymentMode.CASH)) {
            done.setVisibility(View.GONE);
            payNow.setVisibility(View.VISIBLE);
            llTipContainer.setVisibility(View.VISIBLE);
            tvChange.setVisibility(View.GONE);
        } else if (datum.getPaid() == 1 && payingMode.equalsIgnoreCase(Constants.PaymentMode.CASH)) {
            done.setVisibility(View.VISIBLE);
            payNow.setVisibility(View.GONE);
            llTipContainer.setVisibility(View.GONE);
            tvChange.setVisibility(View.GONE);
...
//        tvChange.setVisibility((!isCard && isCash) ? View.GONE : View.VISIBLE);
        ...
        if (payingMode.equalsIgnoreCase(Constants.PaymentMode.CARD)) {

            payNow.setText(getResources().getString(R.string.done)); // set paynow to done button and call payment api
            tvChange.setVisibility(View.GONE);
        }
    }
...
//                            cardHashMap.put("payment_type", Constants.PaymentMode.CARD);
                            cardHashMap.put("payment_mode", Constants.PaymentMode.CARD);
...
//                                cashHashMap.put("payment_type", Constants.PaymentMode.CASH);
                                cashHashMap.put("payment_mode", Constants.PaymentMode.CASH);
                                presenter.payment(cashHashMap);
                            }
                            break;
                        case Constants.PaymentMode.WALLET:
                            showLoading();
                            HashMap<String, Object> walletHashMap = new HashMap<>();
                            walletHashMap.put("request_id", DATUM.getId());
                            walletHashMap.put("tips", tips);
                            walletHashMap.put("payment_mode", Constants.PaymentMode.WALLET);
                            presenter.payment(walletHashMap);
                            break;
    ...
    private void showTipDialog(double totalAmount) {
        ...
        tvSubmit.setOnClickListener(v -> {
            if (!TextUtils.isEmpty(etAmount.getText()) && Double.parseDouble(etAmount.getText().toString()) > 0) {
                ...
                tvTipAmt.setText(getNewNumberFormat(tips));
//                tvTipAmt.setText(String.format("%s %s",
//                        getKey(Objects.requireNonNull(getContext()), "currency"),
//                        getNewNumberFormat(tips)));
                if (payableCal > 0) {
                    llPayable.setVisibility(View.VISIBLE);
                    payable.setText(getNewNumberFormat(payableCal));
//                    payable.setText(String.format("%s %s",
//                            getKey(Objects.requireNonNull(getContext()), "currency"),
            ...
            case PICK_PAYMENT_METHOD:
                if (resultCode == Activity.RESULT_OK) {
                    ...
                    initPaymentView(data.getStringExtra("payment_mode"),
                            data.getStringExtra("card_last_four"), true, data.getIntExtra("use_wallet", 0));
...
//                    brainHashMap.put("payment_type", Constants.PaymentMode.BRAINTREE);
                    brainHashMap.put("payment_mode", Constants.PaymentMode.BRAINTREE);
    ...
    void initPaymentView(String paymentMode, String value, boolean payment, int useWallet) {

        switch (paymentMode) {
            case Constants.PaymentMode.CASH:
                Log.d("1111111111111111111", "Use wallet::::" + useWallet);
                if (useWallet == 1) {
                    tvPaymentMode.setText(getString(R.string.wallet));
                    tvChange.setVisibility(View.GONE);
                    tvWaitingTimeDesc.setVisibility(View.GONE);
                } else tvPaymentMode.setText(getString(R.string.cash));
                break;
            case Constants.PaymentMode.WALLET:
                if (useWallet == 1) {
                    tvPaymentMode.setText(getString(R.string.wallet));
            ...
            case Constants.PaymentMode.CARD:
                if (payment)
                    ...
                    else tvPaymentMode.setText(getString(R.string.card));
                else tvPaymentMode.setText(getString(R.string.card));
                break;
            case Constants.PaymentMode.BRAINTREE:
